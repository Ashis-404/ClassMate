...
import { Plus, Trash2, Calendar, CheckCircle2, ChevronRight, BookOpen, Clock, AlertCircle, User, Award, School, Building } from 'lucide-react';
...
export const Onboarding: React.FC = () => {
  const { user, setUser, setTerm, addSubject, updateSchedule, completeOnboarding } = useApp();
  ...
  const [userType, setUserType] = useState<'College' | 'School' | null>(null);
  ...
  const handleNextStep = () => {
    ...
    if (step === 1) {
      ...
      setTerm({
        ...
        name: termName,
        startDate,
        endDate: new Date(new Date(startDate).setMonth(new Date(startDate).getMonth() + 4)).toISOString().split('T')[0],
  ...
  const removeFromSchedule = (day: string, index: number) => {
    ...
    setScheduleMap(prev => ({ ...prev, [day]: newDaySchedule }));
  };

  const stepTitles = ['Welcome!', 'Academic Setup', 'Your Subjects', 'Weekly Routine'];
  const stepSubtitles = [
    "Let's get to know you a little.",
    `Let's configure your ${user?.type === 'College' ? 'semester' : 'academic year'}.`,
    'What are you studying this term?',
    'Map out your standard week.'
  ];
        ...
        <AnimatePresence mode='wait'>
          {step === 1 && (
            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
              <div className="space-y-2">
                <label className="text-sm text-gray-400 uppercase tracking-wider font-bold">What should we call you?</label>
                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Alex Doe" className="w-full bg-surface p-4 rounded-xl border border-white/10 outline-none focus:border-neon-purple transition-colors" />
              </div>
              <div className="space-y-2">
                 <label className="text-sm text-gray-400 uppercase tracking-wider font-bold">I am currently in</label>
                 <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => setUserType('School')} className={`flex flex-col items-center justify-center gap-2 p-6 rounded-2xl border-2 transition-all ${userType === 'School' ? 'bg-neon-cyan/10 border-neon-cyan' : 'bg-surface border-white/10 hover:border-white/20'}`}>
                        <School size={32} className={`${userType === 'School' ? 'text-neon-cyan' : 'text-gray-400'}`} />
                        <span className="font-bold text-lg">School</span>
                    </button>
                     <button onClick={() => setUserType('College')} className={`flex flex-col items-center justify-center gap-2 p-6 rounded-2xl border-2 transition-all ${userType === 'College' ? 'bg-neon-purple/10 border-neon-purple' : 'bg-surface border-white/10 hover:border-white/20'}`}>
                        <Building size={32} className={`${userType === 'College' ? 'text-neon-purple' : 'text-gray-400'}`} />
              ...
              <div className="bg-surface p-5 rounded-2xl border border-white/10 space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 bg-surface-light rounded-xl px-3 border border-white/5 focus-within:border-neon-purple transition-colors">
                    ...
                    <input type="text" placeholder={user?.type === 'College' ? "e.g. Advanced Calculus" : "e.g. Mathematics"} value={tempSubject} onChange={e => setTempSubject(e.target.value)} className="flex-1 bg-transparent py-3 outline-none"/>
                  </div>
                </div>
                <div className="flex gap-4">
                  <div className="flex-1 space-y-2">
                    <label className="text-xs text-gray-400 font-bold uppercase tracking-wider">Type</label>
                    <div className="flex gap-2">{[SubjectType.THEORY, SubjectType.LAB].map(type => (<button key={type} onClick={() => setTempType(type)} className={`flex-1 py-3 rounded-xl text-sm font-medium border transition-colors ${tempType === type ? 'bg-neon-purple/20 border-neon-purple text-neon-purple' : 'bg-surface-light border-white/5 text-gray-500 hover:border-white/20'}`}>{type}</button>))}
                    </div>
                  </div>
                  {user?.type === 'College' && (
                    <div className="w-1/3 space-y-2">
                      ...
                      <div className="flex items-center gap-2 bg-surface-light rounded-xl px-3 border border-white/5 focus-within:border-neon-purple transition-colors">
                        ...
                        <input type="number" min="0" value={tempCredits} onChange={e => setTempCredits(parseInt(e.target.value) || 0)} className="w-full bg-transparent py-3 outline-none" />
                      </div>
                    </div>
                  )}
                </div>
                <div className="space-y-2">
                  <label className="text-xs text-gray-400 font-bold uppercase tracking-wider">{user?.type === 'College' ? 'Lecturer (Optional)' : 'Teacher (Optional)'}</label>
                  <div className="flex items-center gap-2 bg-surface-light rounded-xl px-3 border border-white/5 focus-within:border-neon-purple transition-colors">
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">{days.map(d => (<button key={d} onClick={() => setSelectedDay(d)} className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors ${selectedDay === d ? 'bg-white text-void' : 'bg-surface text-gray-500'}`}>{d}</button>))}
              </div>
